...
This project is wired to show the "Excel AI Assistant" UI (from the `addon test` folder) in the task pane. Open Excel, click Home > Commands Group > "Show Task Pane" to open it. Configure your API settings in the panel at the top, ask questions in chat, and use the "Read Data" / "Insert to Excel" actions. If you don't add an API key, the assistant will still demo interactions with sample responses.

### Quick start from terminal (optional)

If you prefer the terminal, you can run the dev server and sideload into Excel with one command:

```powershell
npm run start -- desktop --app excel
```

Note: the first run may prompt to allow a loopback exemption for the WebView. If you see "Access Denied", run VS Code as Administrator once and start again.
...
## Slot 4: Insert Template Sheet

In the task pane, open the Sidebar and select "Slot 4". Click the button "Insert Template to New Sheet" to add a new worksheet pre-populated with a basic table template:

- Columns: Item, Category, Quantity, Unit Price, Total, Notes
- Total column has formulas (Quantity × Unit Price)
- A formatted Excel Table is created and headers are frozen
- Currency formatting is applied to Unit Price/Total
- A summary total is added below the table

If a sheet named "Template" already exists, the add-in will create "Template 2", "Template 3", and so on.
